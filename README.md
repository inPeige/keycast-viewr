...
实时展示键盘和鼠标操作的可视化小工具（Windows / macOS）。**无边框透明悬浮窗**，始终浮在其他软件之上且不抢焦点。

![运行截图](jietu.png)

- **上方左侧**：一个鼠标图形，鼠标左/右/中键、滚轮实时高亮。
- **上方右侧**：放大显示当前正在按下的组合键（例如按下 `Shift+Win+A` 就显示三个大按键 `Shift` `+` `Win` `+` `A`）。
- **下方**：铺满整行的虚拟键盘（TKL 布局），实时高亮你正在按下的按键。
- 背景半透明可调（0%–100%），拖窗口本体移动，右下角手柄缩放，右键菜单管理。
...
我媳妇是大学老师，这学期开始带《计算机基础》课。她在给学生讲课时，想把自己的键盘、鼠标操作实时展示在屏幕上，方便学生跟着看、跟着学，所以我做了这个小软件。

如果它对你也有用，麻烦点个 **Star ⭐** 支持一下，谢谢！
